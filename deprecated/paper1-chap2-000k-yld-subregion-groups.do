** HEADER -----------------------------------------------------
**  DO-FILE METADATA
    //  algorithm name			    chap2-000k-yld-subregion.do
    ...
    ** Close any open log file and open a new log file
    capture log close
    log using "`logpath'\chap2-000k-yld-subregion", replace
** HEADER -----------------------------------------------------
...
** ------------------------------------------
** Loading yld dataset for the Americas only 
** Americas (AMR)
**  1100    Cardiovascular 
...
* TODO: Change restriction for each disease group
    ...
    ** Ensure we don't double count population for mental health and neurological (820 940)
    ...
    collapse (sum) yld pop, by(ghecause year paho_subregion sex age)
...
collapse (sum) yld pop, by(year ghecause paho_subregion sex age18 agroup)
...
///             keep if sex==`y'
///             keep if ghecause==`z' 
///             dstdize yld pop age18, by(paho_subregion) using(`who_std')
...
label data "Crude and Adjusted yld rates: PAHO sub-regions"
save "`datapath'\from-who\chap2_000k_yld_subregion_groups", replace
...
replace age18 = 17 if age==80
replace age18 = 18 if age==85
collapse (sum) yld pop, by(year ghecause paho_subregion age18 agroup)
...
** replace pop = pop/1000000
label data "Crude and Adjusted yld rates: PAHO sub-regions"
save "`datapath'\from-who\chap2_000k_yld_subregion_groups_both", replace
