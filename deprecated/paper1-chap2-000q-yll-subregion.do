** HEADER -----------------------------------------------------
**  DO-FILE METADATA
    //  algorithm name			    chap2-000q-yll-subregion.do
    ...
    ** Close any open log file and open a new log file
    capture log close
    log using "`logpath'\chap2-000q-yll-subregion", replace
** HEADER -----------------------------------------------------
...
** ------------------------------------------
** Loading yll dataset for the Americas only 
** Americas (AMR)
**  1100    Cardiovascular 
...
* TODO: Change restriction for each disease group
    ...
    ** Collapse from countries to subregions
    collapse (sum) yll pop, by(ghecause year paho_subregion sex age)
...
collapse (sum) yll pop, by(year ghecause paho_subregion sex age18 agroup)

** Join the yll dataset with the WHO STD population
...
label var yll "Count of all yll"
...
///             keep if sex==`y'
///             keep if ghecause==`z' 
///             dstdize yll pop age18, by(paho_subregion) using(`who_std')
...
label data "Crude and Adjusted yll rates: PAHO sub-regions"
save "`datapath'\from-who\chap2_000q_yll_subregion", replace
...
replace age18 = 17 if age==80
replace age18 = 18 if age==85
collapse (sum) yll pop, by(year ghecause paho_subregion age18 agroup)
...
** replace pop = pop/1000000
label data "Crude and Adjusted yll rates: PAHO sub-regions"
save "`datapath'\from-who\chap2_000q_yll_subregion_both", replace
