...
**!  ------------------------------------------------------------
**!  PART 1
**!  REGION-LEVEL DALY DATA PREPARATION
**!  SEX-SPECIFIC
**!  ------------------------------------------------------------
...
** -----------------------------------
** 15-NOV-2022
** Keep lower deaths limit
drop daly daly_up
rename daly_low daly
** -----------------------------------
** Collapse from 18 to 17 5 year groups.
** This means 80+ instead of 85+ 
collapse (sum) daly pop, by(iso3c iso3n iso3 year age sex ghecause un_region un_subregion who_region paho_subregion)
...
preserve
    ...
    rename pop t1
    collapse (sum) daly (mean) pop=t1 , by(year sex who_region age18 agroup subgroup)
    keep if subgroup == 1
    gen ghecause = 50
    drop subgroup
...
** DEATHS and POPULATION (TOTALS FOR THE AMERICAS): 
** WOMEN and MEN SEPARATELY
** USED IN FIGURE 3 (and associated)
** save "`datapath'\from-who\paper1-chap3_byage_groups_malefemale_daly", replace
   rename daly daly_lo
   save "`datapath'\from-who\daly1_lo", replace
...
** save "`datapath'\from-who\paper1-chap2_000e_daly_region_groups", replace
   rename cases cases_lo 
   rename crate crate_lo 
   rename arate arate_lo
   drop aupp alow    
   save "`datapath'\from-who\dalyrate1_lo", replace
...
preserve
    ...
    replace subgroup = 1 if ghecause>=400 & ghecause<=900
    rename pop t1
    collapse (sum) daly (mean) pop=t1 , by(year who_region age18 agroup subgroup)
    keep if subgroup == 1
    gen ghecause = 50
    drop subgroup
    save `combined' , replace
...
** USED IN FIGURE 3 (and associated)
** save "`datapath'\from-who\paper1-chap3_byage_groups_both_daly", replace
   rename daly daly_lo
   save "`datapath'\from-who\daly2_lo", replace
...
** replace pop = pop/1000000
label data "Crude and Adjusted DALYs: WHO regions"
** save "`datapath'\from-who\paper1-chap2_000e_daly_region_groups_both", replace
   rename cases cases_lo 
   rename crate crate_lo 
   rename arate arate_lo
   drop aupp alow    
   save "`datapath'\from-who\dalyrate2_lo", replace




** Join
use "`datapath'\from-who\dalyrate2_lo" , clear
append using "`datapath'\from-who\dalyrate1_lo"
...
** save "`datapath'\from-who\paper1-chap2_000_daly", replace
save "`datapath'\from-who\dalyrate_12_lo", replace
...
**!  ------------------------------------------------------------
**!  PART 3
**!  COUNTRY-LEVEL DALY DATA PREPARATION
**!  SEX-SPECIFIC
**!  ------------------------------------------------------------
...
preserve
    ...
    replace subgroup = 1 if ghecause>=400 & ghecause<=900
    rename pop t1
    collapse (sum) daly (mean) pop=t1 , by(sex year paho_subregion iso3c iso3n age18 agroup subgroup)
    keep if subgroup == 1
    gen ghecause = 50
    drop subgroup
    save `combined' , replace
...
** 18 age groups
** save "`datapath'\from-who\paper1-chap3_byage_country_groups_malefemale_daly", replace
   rename daly daly_lo
   save "`datapath'\from-who\daly3_lo", replace
...
** save "`datapath'\from-who\paper1-chap3_byage_country_groups_both_daly", replace
   rename daly daly_lo
   save "`datapath'\from-who\daly4_lo", replace
