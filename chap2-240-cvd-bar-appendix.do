    ...
    ** REPORTS and Other outputs
    local outputpath "C:\yasuki\Sync\output\analyse-write\w003\outputs"
                            ...
                            ** Bring scalar 6 back to be from 0 x-axis
                            ...
                            /// **! This needs adapting for each CoD group 
                            /// ** Abbreviate long country names
                            /// #delimit ; 
                            ///     label define region6    27 "St.Vincent & Gren"
                            ///                             24 "Antigua & Barbuda"
                            ///                             15 "Trinidad & Tobago"
                            ///                             31 "Dominican Rep"
                            ///                             11 "Americas", modify;
...
#delimit ;  
	gr twoway 
        ...
        (sc value6 x6, msymbol(i) mlabel(value6) mlabsize(6) mlabcol(gs8) mlabp(0))
           ...
           text(35.5  -100 "IoD:"                       ,  place(c) size(11) color("`cvd1'") just(center))
           text(35.75 -175 "`teardrop'"                ,  place(c) size(5) color("`cvd1'") just(center))
...
graph export "`outputpath'\reports\2024-edits\graphics\figA4-1-B.svg", replace
graph export "`outputpath'\reports\2024-edits\graphics\figA4-1-B.pdf", replace
...
#delimit cr	
** Export to Vector Graphic
graph export "`outputpath'\reports\2024-edits\graphics\figA4-1-A.svg", replace
graph export "`outputpath'\reports\2024-edits\graphics\figA4-1-A.pdf", replace
...
#delimit ;  
	gr twoway 
		/// country values
        (rbar origin1 scaler6 region6 if cod==6 & region!=2000, horizontal barw(0.6) fcol("`cvd2'") lcol("`cvd2'") lw(0.1))           
        (rbar origin1 scaler6 region6 if cod==6 & region==2000, horizontal barw(0.6) fcol(gs0) lcol(gs0) lw(0.1))           
        (sc value6 x6, msymbol(i) mlabel(ad6s) mlabsize(6) mlabcol(gs8) mlabp(0))
           ...
           text(37 200 "All CVD" ,  place(c) size(7.5) color(gs8) just(center))
            ...
            /// INDEX OF DISPARITY VALUES
           text(35.5  -200 "IoD:"                       ,  place(c) size(11) color("`cvd1'") just(center))
           text(35.75 -350 "`teardrop'"                ,  place(c) size(5) color("`cvd1'") just(center))
           text(35.5  -20 "`id6'"                     ,  place(c) size(11) color("`cvd1'") just(center))
...
#delimit cr	
** Export to Vector Graphic
graph export "`outputpath'\reports\2024-edits\graphics\figA4-1-C.svg", replace
graph export "`outputpath'\reports\2024-edits\graphics\figA4-1-C.pdf", replace
...
#delimit cr	
** Export to Vector Graphic
graph export "`outputpath'\reports\2024-edits\graphics\figA4-1-D.svg", replace
graph export "`outputpath'\reports\2024-edits\graphics\figA4-1-D.pdf", replace