...
** Add World Bank income groups into the dataset
** Only do it for this Grouped mortlaity rate
** To allow a new income grouped stratification for 'Excess Mortality' chart (Chapter 1, Fig 1.8)

**! Have run this on Aug 27 6am
**! Only needed to run once to merge in the income groups
** (A) Prepare the Income Group file
/// tempfile incomegroups
/// import excel using  "X:\OneDrive - The University of the West Indies\Writing\w003\data\from-worldbank\CLASS.xls" , first sheet("Groups") clear
/// keep if GroupCode == "HIC" | GroupCode == "LIC" | GroupCode=="LMC" | GroupCode=="UMC"
/// keep GroupCode CountryCode
/// rename CountryCode iso3c 
/// gen wbincome =      1 if GroupCode=="LIC"
/// replace wbincome =  2 if GroupCode=="LMC"
/// replace wbincome =  3 if GroupCode=="UMC"
/// replace wbincome =  4 if GroupCode=="HIC"
/// label define wbincome_ 1 "lic" 2 "lmc" 3 "hmc" 4 "hic" , modify 
/// label values wbincome wbincome_
/// drop GroupCode 
/// save `incomegroups' , replace
/// forval x = 1(1)6 {
///     use "`datapath'\from-who\who-ghe-deaths-001-who`x'-allcauses", replace
///     merge m:1 iso3c using `incomegroups'
///     drop if _merge==2
///     drop _merge
///     save "`datapath'\from-who\who-ghe-deaths-001-who`x'-allcauses-wb", replace
/// }
...
/// ** Africa (AFR)
/// use "`datapath'\from-who\who-ghe-deaths-001-who1-allcauses-wb", replace
/// 
...
///     rename pop pop_temp 
///     collapse (sum) dths (mean) pop=pop_temp, by(ghecause year wbincome sex age iso3c iso3n paho_subregion)
///     collapse (sum) dths pop, by(ghecause year wbincome sex age)
///     save `afr' , replace
...
/// use "`datapath'\from-who\who-ghe-deaths-001-who3-allcauses-wb", replace
/// * TODO: Change restriction for each disease group
///     #delimit ;
///     keep if     
///                 /// MAJOR DISEASE GROUPS
...
/// 
/// ** Europe (EUR)
/// use "`datapath'\from-who\who-ghe-deaths-001-who4-allcauses-wb", replace
/// * TODO: Change restriction for each disease group
///     #delimit ;
///     keep if     
///                 /// MAJOR DISEASE GROUPS
...
/// use "`datapath'\from-who\who-ghe-deaths-001-who5-allcauses-wb", replace
/// * TODO: Change restriction for each disease group
///     #delimit ;
///     #delimit ;
///     keep if     
...
/// 
/// ** Western Pacific (WPR)
/// use "`datapath'\from-who\who-ghe-deaths-001-who6-allcauses-wb", replace
/// * TODO: Change restriction for each disease group
///     #delimit ;
///     #delimit ;
///     keep if     
...
** Just do this for Americas right now
use `amr', clear 
...
collapse (sum) dths pop, by(year ghecause wbincome sex age18 agroup)
...
** Variable labelling
label var wbincome "World Bank income groups"
...
** Crude rate
    bysort sex year ghecause wbincome: egen num = sum(case)
    bysort sex year ghecause wbincome: egen denom = sum(lpop)
    gen crude = num / denom

** (Ref Pop)/(Local Pop) * (Local Observed Events)
    gen srate1 = rpop / lpop * case 
    bysort sex year ghecause wbincome: egen tsrate1 = sum(srate1)
    bysort sex year ghecause wbincome: egen trpop = sum(rpop)
    bysort sex year ghecause wbincome: egen tlpop = sum(lpop)
    sort age18
    ** Per 10,000
...
** DSR: 1 / sum(refpop) * sum(refpop*case/localpop) 
    bysort sex year ghecause wbincome: egen t1a = sum(rpop)
    gen  t1b = 1/t1a
    gen t2a = rpop * case / lpop
    bysort sex year ghecause wbincome: egen t2b = sum(t2a)
    gen dsr = t1b * t2b
...
** DSR 95%CI
    **  DSR
    gen ci1 = dsr 
    **  Case(lower)
    bysort sex year ghecause wbincome: egen ol1 = sum(case)
    gen ol2 = 1 / (9*ol1)
    gen ol3 = 1.96 / (3 * sqrt(ol1))
    gen ol4 = ol1 * (1- ol2 - ol3)^3
    **  Case(upper)
    bysort sex year ghecause wbincome: egen ou1 = sum(case)
    gen ou2 = 1 / (9*(ou1 + 1))
    gen ou3 = 1.96 / (3 * sqrt(ou1 + 1))
    gen ou4 = (ou1+1) * (1 - ou2 + ou3)^3
    **  Var(DSR)
    gen var1 = rpop^2 * case / lpop^2
    bysort sex year ghecause wbincome: egen var2 = sum(var1)
    bysort sex year ghecause wbincome: egen var3 = sum(rpop)
    ...
    ** Collapse out the local population
    collapse (sum) cases lpop (mean) crate=crude arate=dsr aupp=cu alow=cl, by(sex year ghecause wbincome)  
...
///             keep if sex==`y'
///             keep if ghecause==`z' 
///             dstdize deaths pop age18, by(wbincome) using(`who_std')
...
label var region "WHO region / PAHO subregion / WB region"

** Variable level labelling
* Regions
recode region 1=11000 2=12000 3=13000 4=14000

#delimit ;
label define region_    11000 "LIC"
                        12000 "LMC"
                        13000 "HMC"
                        14000 "HIC" , modify; 
#delimit cr 
...
save "`datapath'\from-who\chap2_000a_mr_region_groups_wb", replace
...
replace age18 = 17 if age==80
replace age18 = 18 if age==85
collapse (sum) dths pop, by(year ghecause wbincome age18 agroup)
...
** Crude rate
    bysort year ghecause wbincome: egen num = sum(case)
    bysort year ghecause wbincome: egen denom = sum(lpop)
    gen crude = num / denom

** (Ref Pop)/(Local Pop) * (Local Observed Events)
    gen srate1 = rpop / lpop * case 
    bysort year ghecause wbincome: egen tsrate1 = sum(srate1)
    bysort year ghecause wbincome: egen trpop = sum(rpop)
    bysort year ghecause wbincome: egen tlpop = sum(lpop)
    sort age18
    ** Per 10,000
    gen rate = tsrate1 / trpop
...
** Method
** DSR: 1 / sum(refpop) * sum(refpop*case/localpop) 
    bysort year ghecause wbincome: egen t1a = sum(rpop)
    gen  t1b = 1/t1a
    gen t2a = rpop * case / lpop
    bysort year ghecause wbincome: egen t2b = sum(t2a)
    gen dsr = t1b * t2b
...
** DSR 95%CI
    **  DSR
    gen ci1 = dsr 
    **  Case(lower)
    bysort year ghecause wbincome: egen ol1 = sum(case)
    gen ol2 = 1 / (9*ol1)
    gen ol3 = 1.96 / (3 * sqrt(ol1))
    gen ol4 = ol1 * (1- ol2 - ol3)^3
    **  Case(upper)
    bysort year ghecause wbincome: egen ou1 = sum(case)
    gen ou2 = 1 / (9*(ou1 + 1))
    gen ou3 = 1.96 / (3 * sqrt(ou1 + 1))
    gen ou4 = (ou1+1) * (1 - ou2 + ou3)^3
    **  Var(DSR)
    gen var1 = rpop^2 * case / lpop^2
    bysort year ghecause wbincome: egen var2 = sum(var1)
    bysort year ghecause wbincome: egen var3 = sum(rpop)
    gen var4 = var2 / (var3 ^2)
    **  DSR(lower)
    gen cl1 = dsr
    gen cl = cl1 + sqrt(var4/ol1) * (ol4 - ol1)
    ...
    ** Collapse out the local population
    collapse (sum) cases lpop (mean) crate=crude arate=dsr aupp=cu alow=cl, by(year ghecause wbincome)  
...
label var year "Year of mortality rate"
label var ghecause "Broad causes of death"
label var region "WHO region / PAHO subregion / WB regions"
...
** replace pop = pop/1000000
label data "Crude and Adjusted mortality rates: WHO regions"
save "`datapath'\from-who\chap2_000a_mr_region_groups_both_wb", replace
