...
local outer1a 0 0     7 0 
local outer1b 0 0    0 48 
local outer1c 7 2    7 46
local outer2a  7 0    15 0   
local outer2c  15 2    15 46
local outer3a  15 0    23 0   
local outer3c  23 2    23 46
local outer4a  23 0    31 0   
local outer4c  31 2    31 46
local outer5a  31 0    39 0   
local outer5c  39 0    39 48
...
#delimit ;
	gr twoway 
        (scatteri `inner1' , recast(area) lw(0.25) lc(gs8) fc(gs14) lp(l))
        (scatteri `inner2' , recast(area) lw(0.25) lc(gs8) fc(gs14) lp(l))
        (scatteri `inner3' , recast(area) lw(0.25) lc(gs8) fc(gs14) lp(l))
        (scatteri `inner4' , recast(area) lw(0.25) lc(gs8) fc(gs14) lp(l))
        (scatteri `inner5' , recast(area) lw(0.25) lc(gs8) fc(gs14) lp(l))

        (scatteri `outer1a' , recast(line) lw(0.25) lc(gs8) fc(none) lp(l))
        (scatteri `outer1b' , recast(line) lw(0.25) lc(gs8) fc(none) lp(l))
        (scatteri `outer1c' , recast(line) lw(0.25) lc(gs8) fc(none) lp("-"))
        (scatteri `outer2a' , recast(line) lw(0.25) lc(gs8) fc(none) lp(l))
        (scatteri `outer2c' , recast(line) lw(0.25) lc(gs8) fc(none) lp("-"))
        (scatteri `outer3a' , recast(line) lw(0.25) lc(gs8) fc(none) lp(l))
        (scatteri `outer3c' , recast(line) lw(0.25) lc(gs8) fc(none) lp("-"))
        (scatteri `outer4a' , recast(line) lw(0.25) lc(gs8) fc(none) lp(l))
        (scatteri `outer4c' , recast(line) lw(0.25) lc(gs8) fc(none) lp("-"))
        (scatteri `outer5a' , recast(line) lw(0.25) lc(gs8) fc(none) lp(l))
        (scatteri `outer5c' , recast(line) lw(0.25) lc(gs8) fc(none) lp(l))

        /// Horizontal lines
        (rbar line_lo line_hi uid1, horizontal fc(gs13)   barw(0.1) lw(none))

		/// Dependency Ratio Points - 33 countries
        (sc uid1 dratio1980  , msize(3) m(o) mlc("`bl2'") mfc("`bl2'%75") mlw(0.1))
        (sc uid1 dratio2020  , msize(3) m(o) mlc("`gry'") mfc("`gry'") mlw(0.1))
        (sc uid1 dratio2060  , msize(3) m(o) mlc("`red'") mfc("`red'%75") mlw(0.1))
		...
		,
			...
			;
#delimit cr	
graph export "`outputpath'/figure2.png", replace width(4000)
...
** FIGURES
    ...
    putpdf text ("Figure 2"), bold font("Calibri Light", 12)
    putpdf table t2 = (1,1), width(85%) halign(center) border(all,nil) 
    putpdf table t2(1,1)=image("`outputpath'/figure2.png")

** Save the PDF
    local c_date = c(current_date)
    local date_string = subinstr("`c_date'", " ", "", .)
    putpdf save "`outputpath'/figure2", replace
