...
** Direct Stand from first principles

** Merge WHO Standard Population
merge m:1 age18 using `who_std'
** (Ref Pop)/(Local Pop) * (Local Observed Events)
gen srate1 = pop / lpop * daly 
bysort year ghecause : egen tsrate1 = sum(srate1)
bysort year ghecause : egen tpop = sum(pop)
bysort year ghecause : egen tlpop = sum(lpop)
sort year ghecause age18
gen rate = tsrate1 / tpop * 100000
...
** Direct standardization 
        rename daly dalyt
        /// gen double daly = int(dalyt*10000000000) 
        gen double dalyt2 = dalyt * 10000000000
        gen double daly = round(dalyt2) 
        format dalyt dalyt2 daly %35.1fc
        label var daly "dalys rounded to nearest integer" 
        rename pop popt 
        gen double pop = pop * 10000000000
        /// gen double pop = int(popt*10000000000) 
        * Save dataset ready for direct standardization 
        drop if ghecause<400
        tempfile for_mr
        save `for_mr' , replace
** 2019, Male, Communicable Disease
forval x = 2000(19)2019 {
        * TODO: Change next line for each disease group
        forval z = 400(100)500 {
            use `for_mr' , clear 
            tempfile results
            keep if year==`x' 
            keep if ghecause==`z' 
            dstdize daly pop age18, by(who_region) using(`who_std')
            matrix m`x'_`z' = r(crude) \ r(adj) \ r(Nobs)
