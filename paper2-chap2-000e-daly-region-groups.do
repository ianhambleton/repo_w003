...
** WORLD
use "`datapath'\from-who\who-ghe-daly-001-world-allcauses", replace
** Collapse from 18 to 17 5 year groups.
** This means 80+ instead of 85+ 
collapse (sum) daly daly_low daly_up pop, by(iso3c iso3n iso3 year age sex ghecause un_region un_subregion who_region paho_subregion)
* TODO: Change restriction for each disease group
    ...
    ** Ensure we don't double count population for mental health and neurological (820 940)
    rename pop pop_temp 
    collapse (sum) daly (mean) pop=pop_temp, by(ghecause year who_region sex age iso3c iso3n paho_subregion)
    collapse (sum) daly pop, by(ghecause year sex age)
    gen who_region = 7 
    save `world' , replace
...
save "`datapath'\paper2-inj\paper2_daly_groups_byage_bysex", replace
...
save "`datapath'\paper2-inj\paper2_chap2_000e_daly_region_groups", replace
...
** Use in PAPER 2
** DEATHS in 18 age groups
save "`datapath'\paper2-inj\paper2_daly_groups_byage", replace
...
** replace pop = pop/1000000
label data "Crude and Adjusted DALYs: WHO regions"
save "`datapath'\paper2-inj\paper2_chap2_000e_daly_region_groups_both", replace
