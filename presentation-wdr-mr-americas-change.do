...
**  DO-FILE METADATA
    ...
    ** Close any open log file and open a new log file
    capture log close
    log using "`logpath'\presentation-wdr-mr-change", replace
** HEADER -----------------------------------------------------

** Original DO file --> chap3-100-mr-change
...
** Keeping the TOP 5 and BOTTOM 4 conditions
keep if (yorder>=1 & yorder<=5) | (yorder>=26 & yorder<=29)

recode yorder1 26=6 27=7 28=8 29=9
...
gen yorder2 = _n 
order yorder yorder2 cod mr_ac_gr1 mr_ac_gr2

#delimit ;
label define yorder2_ 
        1  "IHD"
        2  "Stroke"
        3  "Lung cancer"
        4  "COPD"
        ...
        9 "Dementias", modify;
#delimit cr
label values yorder2 yorder2_

local line1 9 0 1 0
local line2 5.5 -35 5.5 10
** Triangles & associated text
local outer1 1.75 23   3.2 19.5    1.75 15    1.75 23
local outer2 7.75 26.5   6.2 22.5    7.75 18.5    7.75 26.5

** GRAPHIC
#delimit ;  
	gr twoway 
		/// country values
        (rbar origin1 mr_ac_gr1 yorder2 if mr_pc<0, horizontal barw(0.6) fcol("`improve'") lcol("`improve'") lw(0.1))           
        (rbar origin2 mr_ac_gr2 yorder2 if mr_pc>=0, horizontal barw(0.6) fcol("`worsen'") lcol("`worsen'") lw(0.1))           
        (sc yorder1 xlocation1, msymbol(i) mlabel(yorder1) mlabsize(4) mlabcol(gs3) mlabp(0))
        (scatteri `line1' , recast(line) lw(0.2) lc("`gry1'%50") fc("`gry1'%50") lp("-") )
        /// (scatteri `line2' , recast(line) lw(0.2) lc("`gry2'%50") fc("`gry2'%50") lp("l") )
        (scatteri -0.5 -25 "Absolute change (2000 to 2019)" , msymbol(i) mlabpos(0) mlabcol(gs3) mlabsize(6) mlabangle(0))
        ///(scatteri `outer1' , recast(area) lw(none) lc("`improve'%65") fc("`improve'%65")  )
        ///(scatteri `outer2' , recast(area) lw(none) lc("`worsen'%45") fc("`worsen'%45")  )
                		,
			...
			xlab(none, 
            notick labs(4.5) tlc(gs0) labc(gs3) notick nogrid glc(gs16))
			xscale(noline range(-35(1)55)) 
			xtitle("", size(2) color(gs0) margin(l=0 r=0 t=0 b=0)) 
			
			ylab(1(1)9, valuelabel
			labc(gs3) labs(4.5) tstyle(major_notick) nogrid glc(gs16) angle(0) format(%9.0f) labgap(2))
			yscale(reverse noline lw(vthin) range(1(1)9)) 
			ytitle("", size(2) margin(l=0 r=0 t=0 b=0)) 

            ///text(3 20 "$sch_fewer fewer deaths" "per 100 000", place(e) size(2.5) color("`improve'") just(center) margin(l=2 r=2 t=2 b=2))
            ///text(6.5 23 "$sch_more more deaths" "per 100 000", place(e) size(2.5) color("`worsen'") just(center) margin(l=2 r=2 t=2 b=2))
...
#delimit ;  
	gr twoway 
        ...
        (scatteri `line1' , recast(line) lw(0.2) lc("`gry1'%50") fc("`gry1'%50") lp("-") )
        /// (scatteri `line2' , recast(line) lw(0.2) lc("`gry2'%50") fc("`gry2'%50") lp("l") )
        (scatteri -0.5 -25 "Absolute change (2000 to 2019)" , msymbol(i) mlabpos(0) mlabcol(ggs3s8) mlabsize(6) mlabangle(0))
        (scatteri `outer1' , recast(area) lw(none) lc("`improve'%65") fc("`improve'%65")  )
        (scatteri `outer2' , recast(area) lw(none) lc("`worsen'%45") fc("`worsen'%45")  )
                		,
			plotregion(c(gs16) ic(gs16) ilw(thin) lw(thin)) 		
            ...
            text(3 20 "$sch_fewer fewer deaths" "per 100 000", place(e) size(4.5) color("`improve'") just(center) margin(l=2 r=2 t=2 b=2))
            text(6.5 23 "$sch_more more deaths" "per 100 000", place(e) size(4.5) color("`worsen'") just(center) margin(l=2 r=2 t=2 b=2))
