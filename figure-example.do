...
** Save minimal dataset for teaching purposes
label data "Life table data from WHO GHO (2000-2019) 
drop cname country wbregion
drop if region=="WB_HI" | region=="WB_LI" | region=="WB_LMI" | region=="WB_UMI"
label var region "global regions" 
label var agroup "Age groups (only<1 kept = LE at birth)"
label var metric35 "Life expectancy (LE)" 
label var metric100 "Healthy life expectancy (HALE)" 
label var yr1 "x-axis location for regional panels"
save "`datapath'\from-who\lifetables\who-lifetable-001", replace
...
export excel "`outputpath'\reports\2024-edits\graphics\chap1_data.xlsx", sheet("figure-1", replace) first(var)

*/

** Version THREE
** Just the World and The Americas - for Presentation - Sep 2024

** New x-axis values to represent YEARS
gen yr2 = .
replace yr2 = 1 if year==2000 
replace yr2 = 2 if year==2005 
replace yr2 = 3 if year==2010 
replace yr2 = 4 if year==2015 
replace yr2 = 5 if year==2019 
replace yr2 = yr2 + 5 if region=="GLOBAL"

** Jitter men by a fraction to improve visual
replace yr2 = yr2 - 0.2 if sex==1 
replace yr2 = yr2 + 0.2 if sex==2 

** Legend outer limits for graphing 
local outer1 80 5.8 78 5.8 78 6.3 80 6.3 80 5.8
local outer2 80 8.1 78 8.1 78 8.6 80 8.6 80 8.1  
...
#delimit ;
	gr twoway 
	    ...
	    (rarea metric35 metric100 yr2 if region=="AMR" & sex==1 & agroup==1 , lw(none) color("`women'%75"))
		(rarea metric35 metric100 yr2 if region=="AMR" & sex==2 & agroup==1 , lw(none) color("`men'%75"))

	    (rarea metric35 metric100 yr2 if region=="GLOBAL" & sex==1 & agroup==1 , lw(none) color("`women'%75"))
		(rarea metric35 metric100 yr2 if region=="GLOBAL" & sex==2 & agroup==1 , lw(none) color("`men'%75"))

		/// The Americas
		(line metric35 yr2 if region=="AMR" & sex==2 & agroup==1 , lw(0.4) lc("`men'"))
		(line metric35 yr2 if region=="AMR" & sex==1 & agroup==1 , lw(0.4) lc("`women'"))
		(line metric100 yr2 if region=="AMR" & sex==2 & agroup==1 , lp("-") lw(0.4) lc("`men'"))
		(line metric100 yr2 if region=="AMR" & sex==1 & agroup==1 , lp("-") lw(0.4) lc("`women'"))

		/// World
		(line metric35 yr2 if region=="GLOBAL" & sex==2 & agroup==1 , lw(0.4) lc("`men'"))
		(line metric35 yr2 if region=="GLOBAL" & sex==1 & agroup==1 , lw(0.4) lc("`women'"))
		(line metric100 yr2 if region=="GLOBAL" & sex==2 & agroup==1 , lp("-") lw(0.4) lc("`men'"))
		(line metric100 yr2 if region=="GLOBAL" & sex==1 & agroup==1 , lp("-") lw(0.4) lc("`women'"))

        /// droplines
        (function y=85, range(1 10.5) lc(gs12) dropline(5.5 10.5))

        /// Legend
        (function y=82, range(5.8 6.3) lc(gs10) lw(0.4))
        (function y=82, range(8.1 8.6) lp("-") lc(gs10) lw(0.4))
        (scatteri `outer1' , recast(area) lw(none) lc("`women'%75") fc("`women'%75")  )
        (scatteri `outer2' , recast(area) lw(none) lc("`men'%75") fc("`men'%75")  )

		/// X-Axis lines
        (function y=54, range(1 5) lc(gs8) lw(0.2))
        (function y=54, range(6 10) lc(gs8) lw(0.2))
		,
			plotregion(c(gs16) ic(gs16) ilw(thin) lw(thin) margin(l=2 r=2 b=0 t=0)) 		
			graphregion(color(gs16) ic(gs16) ilw(thin) lw(thin) margin(l=2 r=2 b=0 t=0)) 
			ysize(12) xsize(16)
			...
			ylab(55(5)85,
			valuelabel labc(gs0) labs(4.5) tlc(gs8) nogrid glc(gs16) angle(0) format(%9.0f))
			yscale(lw(vthin) lc(gs8) range(55(5)90) noextend) 
			ytitle(" ", color(gs8) size(4.5) margin(l=1 r=1 t=1 b=1)) 

            /// Region Titles 
            text(87 8 "World",  place(c) size(4.5) color(gs5))
            text(87 3 "The Americas",  place(c) size(4.5) color(gs5))  
            /// Legend Text
            text(82 6.4 "LE",  place(e) size(4) color(gs8))   
            text(82 8.7   "HALE",  place(e) size(4) color(gs8))   
            text(79   6.4 "Women",  place(e) size(4) color(gs8))   
            text(79   8.7   "Men",  place(e) size(4) color(gs8))   
			/// X-Axis text
            text(55 1 "2000",  place(e) size(4.5) color(gs0))
            text(55 5 "2019",  place(w) size(4.5) color(gs0))
            text(55 6 "2000",  place(e) size(4.5) color(gs0))
            text(55 10 "2019",  place(w) size(4.5) color(gs0))
