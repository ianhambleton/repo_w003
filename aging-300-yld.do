...
**!  ------------------------------------------------------------
**!  PART 1
**!  REGION-LEVEL YLD DATA PREPARATION
**!  SEX-SPECIFIC
**!  ------------------------------------------------------------
...
** ------------------------------------------
** Loading YLD datasets for WHO regions 
** ------------------------------------------

tempfile afr amr emr eur sear wpr world
** Africa (AFR)
use "`datapath'\from-who\who-ghe-yld-001-who1-allcauses", replace
...
* TODO: Change restriction for each disease group
    ...
    ** Ensure we don't double count population for mental health and neurological (820 940)
    rename pop pop_temp 
    collapse (sum) yld (mean) pop=pop_temp, by(ghecause year who_region sex age iso3c iso3n paho_subregion)
    collapse (sum) yld pop, by(ghecause year who_region sex age)
    save `afr' , replace
...
use "`datapath'\from-who\who-ghe-yld-001-who3-allcauses", replace
* TODO: Change restriction for each disease group
    #delimit ;
    keep if     
                /// MAJOR DISEASE GROUPS
...
** Europe (EUR)
use "`datapath'\from-who\who-ghe-yld-001-who4-allcauses", replace
* TODO: Change restriction for each disease group
    #delimit ;
    keep if     
                /// MAJOR DISEASE GROUPS
...
use "`datapath'\from-who\who-ghe-yld-001-who5-allcauses", replace
* TODO: Change restriction for each disease group
    #delimit ;
    #delimit ;
    keep if     
...
** Western Pacific (WPR)
use "`datapath'\from-who\who-ghe-yld-001-who6-allcauses", replace
* TODO: Change restriction for each disease group
    #delimit ;
    #delimit ;
    keep if     
...
collapse (sum) yld pop, by(year ghecause who_region sex age18 agroup)

** Join the yld dataset with the WHO STD population
...
label var yld "Count of all YLDs"
...
preserve
    ...
    rename pop t1
    collapse (sum) yld (mean) pop=t1 , by(year sex who_region age18 agroup subgroup)
    keep if subgroup == 1
    gen ghecause = 50
    drop subgroup
...
** save "`datapath'\from-who\chap3_byage_groups_malefemale", replace
   save "`datapath'\from-who\yld1", replace
...
** save "`datapath'\from-who\chap2_000j_yld_region_groups", replace
   save "`datapath'\from-who\yldrate1", replace


** List for TABLE 1.3
preserve
    use "`datapath'\from-who\yldrate1", clear
...
replace age18 = 17 if age==80
replace age18 = 18 if age==85
collapse (sum) yld pop, by(year ghecause who_region age18 agroup)
...
label var agroup "5 broad age groups: young children, youth, young adult, older adult, elderly"
label var age18 "5-year age groups: 18 groups"
label var yld "Count of all yld"
label var pop "PAHO subregional populations" 
format pop %12.0fc 
** label var spop "WHO Standard population: sums to 1 million"
...
preserve
    ...
    replace subgroup = 1 if ghecause>=400 & ghecause<=900
    rename pop t1
    collapse (sum) yld (mean) pop=t1 , by(year who_region age18 agroup subgroup)
    keep if subgroup == 1
    gen ghecause = 50
    drop subgroup
    save `combined' , replace
...
** 18 age groups
** save "`datapath'\from-who\paper1-chap3_byage_groups_both_yld", replace
   save "`datapath'\from-who\yld2", replace
...
** replace pop = pop/1000000
label data "Crude and Adjusted mortality rates: WHO regions"
save "`datapath'\from-who\chap2_000j_yld_region_groups_both", replace
   save "`datapath'\from-who\yldrate2", replace
...
** Join
use "`datapath'\from-who\yldrate2" , clear
append using "`datapath'\from-who\yldrate1"
...
** save "`datapath'\from-who\paper1-chap2_000_yld", replace
save "`datapath'\from-who\yldrate_12", replace
...
**!  ------------------------------------------------------------
**!  PART 4 (we don't use PART 3 for YLD)
**!  COUNTRY-LEVEL YLD DATA PREPARATION
**!  SEX-COMBINED
**!  ------------------------------------------------------------
...
** HEADER -----------------------------------------------------
**  DO-FILE METADATA
    //  algorithm name			    chap2-000m-yld-country.do
    //  project:				    WHO Global Health Estimates
    //  analysts:				    Ian HAMBLETON
    ...
    ** Close any open log file and open a new log file
    capture log close
    log using "`logpath'\chap2-000m-yld-country", replace
** HEADER -----------------------------------------------------
...
** ------------------------------------------
** Loading ylds dataset for the Americas only 
** Americas (AMR)
**  1100    Cardiovascular 
...
collapse (sum) yld yld_low yld_up pop, by(iso3c iso3n iso3 year age sex ghecause un_region un_subregion who_region paho_subregion)

** yld to zero for Haiti in 2010 for natural disasters.
** The earthquale meant that yld > POP, causing problems for the algorithms
** which expects yld < POP
replace yld = 0 if iso3n==332 & year==2010 & ghecause==1510
...
* TODO: Change restriction for each disease group
    ...
    ** Ensure we don't double count population for mental health and neurological (820 940)
    rename pop pop_temp 
    collapse (sum) yld (mean) pop=pop_temp, by(ghecause year who_region sex age iso3c iso3n paho_subregion)
    collapse (sum) yld pop, by(ghecause year sex age iso3n iso3c paho_subregion)
...
collapse (sum) yld pop, by(year ghecause iso3n iso3c paho_subregion age18 agroup)

** Join the ylds dataset with the WHO STD population
** merge m:m age18 using `who_std'
...
preserve
    ...
    replace subgroup = 1 if ghecause>=400 & ghecause<=900
    rename pop t1
    collapse (sum) yld (mean) pop=t1 , by(year paho_subregion iso3c iso3n age18 agroup subgroup)
    keep if subgroup == 1
    gen ghecause = 50
    drop subgroup
    save `combined' , replace
...
** Use in Chapter 3. Population change
** 18 age groups
save "`datapath'\from-who\paper1-chap3_byage_country_groups_both_yld", replace
   save "`datapath'\from-who\yld4", replace
